...
- `trackfi-api`: 
  - requirements:
    ...
    - Docker
  - maven profiles:
    - `embedded`: adds embedded h2 database compatible with `spring-data-jpa`

### Running TrackFi locally
A docker compose file has been added to the project's root directory to run TrackFi locally. Note that `mvn spring-boot:build-image -Pembedded` must be run within the `trackfi-api` directory before attempting to create the stack. This command must be run again for any changes to appear in the resulting docker image.
